...
#define QEICN_new_train QE_RSC_ICON(add-diagram.svg)   // 新建车次

#define QEICN_timetable_quick QE_RSC_ICON(history.svg)   // 速览时刻窗口
#define QEICN_train_info_quick QE_RSC_ICON(info.svg)   // 速览信息窗口
...
#define QEICN_compare_trains QE_RSC_ICON(diff.svg)   // 车次对照
#define QEICN_timetable_diagon QE_RSC_ICON(identify.png)  // 时刻诊断
#define QEICN_compare_diagram QE_RSC_ICON(diff.svg)  // 运行图对比
#define QEICN_section_count QE_RSC_ICON(counter.png)   // 区间对数表
#define QEICN_section_trains QE_RSC_ICON(train.svg)   // 区间车次表
#define QEICN_interval_stat QE_RSC_ICON(data.png)   // 列车区间运行统计

#define QEICN_ruler_paint QE_RSC_ICON(toll.svg)   // 标尺排图
#define QEICN_batch_copy_trains QE_RSC_ICON(copy.svg)   // 批量复制车次
#define QEICN_timetable_interp QE_RSC_ICON(add-diagram.svg)    // 时刻插值
#define QEICN_greedy_paint QE_RSC_ICON(toll.svg)   // 贪心推线
...
#define QEICN_change_page QE_RSC_ICON(switch.svg)   // 切换页面
#define QEICN_edit_page QE_RSC_ICON(edit.svg)  // 编辑运行图页面
#define QEICN_export_page QE_RSC_ICON(application-pdf.svg)   // 导出运行图页面
...
#define QEICN_edit_rail_page QE_RSC_ICON(add-track.svg)   // 编辑线路（运行图页面）
...
#define QEICN_page_copy QE_RSC_ICON(copy.svg)   // 运行图页面副本
...
#define QEICN_change_train QE_RSC_ICON(switch.svg)   // 切换列车
#define QEICN_train_line QE_RSC_ICON(trainline.png)   // 高亮列车运行线
#define QEICN_train_events QE_RSC_ICON(history.svg)    // 列车事件表
#define QEICN_train_line_list QE_RSC_ICON(line-manage.png)   // 运行线一览
#define QEICN_ruler_diff QE_RSC_ICON(ruler.svg)   // 标尺对照
...
#define QEICN_add_train_to_routing QE_RSC_ICON(add-diagram.svg)   // 将列车添加到交路
...
#define QEICN_add_path_to_train QE_RSC_ICON(add-diagram.svg)   // 添加径路到列车
#define QEICN_remove_path_from_train QE_STD_ICON(TrashIcon)   // 从列车移除径路

#define QEICN_interval_exchange QE_RSC_ICON(switch.svg)   // 区间换线
#define QEICN_timetable_adjust QE_RSC_ICON(adjust.png)   // 时刻平移
#define QEICN_timetable_correction QE_RSC_ICON(settings.png)  // 时刻修正
#define QEICN_timetable_simple_interp QE_RSC_ICON(add-diagram.svg)   // 快速推定通过站时刻

#define QEICN_edit_timetable QE_RSC_ICON(timetable.png)   // 编辑时刻表
#define QEICN_edit_train QE_RSC_ICON(edit.svg)   // 编辑列车所有数据

#define QEICN_copy_train QE_RSC_ICON(copy.svg)   // 创建列车副本
#define QEICN_del_train QE_STD_ICON(TrashIcon)   // 删除列车
#define QEICN_close_train_context QE_STD_ICON(DialogCloseButton)   // 关闭列车面板

///////// 工具栏 线路工具 (8) /////////

#define QEICN_change_rail QE_RSC_ICON(switch.svg)   // 切换线路
#define QEICN_edit_rail QE_RSC_ICON(add-track.svg)   // 基线编辑
#define QEICN_edit_skylight QE_RSC_ICON(forbid.png)   // 天窗编辑
#define QEICN_edit_ruler_rail QE_RSC_ICON(ruler.svg)   // 标尺编辑 从线路进入
#define QEICN_add_ruler QE_RSC_ICON(add-diagram.svg)   // 新建标尺
...
#define QEICN_station_events QE_RSC_ICON(electronic-history.svg)  // 车站事件表
#define QEICN_section_events QE_RSC_ICON(diagram.png)   // 断面事件表
#define QEICN_time_snap QE_RSC_ICON(history.svg)   // 运行快照 同一时刻下的所有列车状态

#define QEICN_station_tracks QE_RSC_ICON(add-track.svg)   // 股道分析
#define QEICN_rail_topo QE_RSC_ICON(diagram.png)  // 线路拓扑
#define QEICN_headway_list QE_RSC_ICON(h_expand.png)  // 列车间隔分析
#define QEICN_headway_summary QE_RSC_ICON(tune.svg)  // 列车间隔汇总
...
#define QEICN_copy_railway QE_RSC_ICON(copy.svg)   // 创建基线副本
#define QEICN_close_rail_context QE_STD_ICON(DialogCloseButton)  // 关闭基线面板

///////// 工具栏 标尺管理 (9) /////////

#define QEICN_change_ruler QE_RSC_ICON(switch.svg)   // 切换标尺
#define QEICN_edit_ruler QE_RSC_ICON(edit.svg)    // 编辑标尺
#define QEICN_ruler_from_train QE_RSC_ICON(identify.png)   // 从车次提取标尺
#define QEICN_ruler_from_speed QE_RSC_ICON(history.svg)   // 从速度计算标尺

#define QEICN_merge_ruler QE_STD_ICON(ArrowBack)   // 合并标尺
#define QEICN_import_ruler_csv QE_RSC_ICON(ruler.svg)   // 导入标尺 (CSV)
#define QEICN_export_ruler_csv QE_RSC_ICON(copy.svg)    // 导出标尺 (CSV)
#define QEICN_ordinate_ruler QE_RSC_ICON(ruler.svg)   // 设置为排图标尺

#define QEICN_copy_ruler QE_RSC_ICON(copy.svg)   // 创建标尺副本
#define QEICN_del_ruler QE_STD_ICON(TrashIcon)   // 删除标尺
#define QEICN_close_ruler_context QE_STD_ICON(DialogCloseButton)   // 关闭标尺面板

///////// 工具栏 交路编辑 (0) /////////

#define QEICN_change_routing QE_RSC_ICON(switch.svg)   // 切换交路
#define QEICN_highlight_routing QE_RSC_ICON(trainline.png)   // 高亮显示
#define QEICN_edit_routing QE_RSC_ICON(edit.svg)   // 交路编辑
#define QEICN_split_routing QE_RSC_ICON(copy.svg)   // 交路拆分
...
#define QEICN_routing_mile QE_RSC_ICON(ruler.svg)   // 交路里程表
...
#define QEICN_change_path QE_RSC_ICON(switch.svg)   // 切换列车径路
#define QEICN_edit_path QE_RSC_ICON(edit.svg)   // 编辑 列车径路
#define QEICN_path_trains QE_RSC_ICON(train.svg)   // 列车表
#define QEICN_path_add_trains QE_RSC_ICON(add-diagram.svg)   // 添加列车到径路
#define QEICN_path_clear_trains QE_STD_ICON(TabCloseButton)   // 清空列车

#define QEICN_del_path QE_STD_ICON(TrashIcon)   // 删除径路
#define QEICN_copy_path QE_RSC_ICON(copy.svg)   // 创建径路副本
#define QEICN_close_path_context QE_STD_ICON(DialogCloseButton)  // 关闭径路面板

///////// 工具栏 路网管理 /////////

#define QEICN_rail_db_toggle QE_RSC_ICON(application-sql.svg)   // 线路数据库面板开关
...
#define QEICN_ruler_db QE_RSC_ICON(ruler.svg)   // 编辑标尺
...
#define QEICN_train_editor_change QE_RSC_ICON(switch.svg)   // 列车编辑页面 切换车次
#define QEICN_train_editor_sync QE_RSC_ICON(exchange1.png)    // 列车编辑页面 保持与选择同步

#define QEICN_routing_editor_change QE_RSC_ICON(switch.svg)   // 交路编辑页面 切换车次
